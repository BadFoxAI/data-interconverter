...
use js_sys::{BigInt as JsBigInt}; // JsString removed as not directly used in API signatures now
use web_sys::console;
use num_bigint::{BigInt, Sign};
use num_traits::{Zero, One, Signed, ToPrimitive, Pow};
use anyhow::{Result, bail, anyhow};
use lazy_static::lazy_static;
use serde::{Serialize, Deserialize}; // Used by serde_wasm_bindgen for Vec<BigInt>
use std::str::FromStr;
...
fn calculate_min_sequence_length_rust(index: &BigInt, bit_depth: u32) -> u32 {
    ...
    if bit_depth == 0 { return if index.is_zero() { 0 } else { u32::MAX }; } // Or bail with error

    let sequence_element_base: BigInt = BigInt::from(2).pow(bit_depth);
    if sequence_element_base <= BigInt::one() { return if index.is_zero() { 0 } else { u32::MAX }; } // e.g. bit_depth = 0
