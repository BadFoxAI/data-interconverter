...
use num_traits::{Zero, One, ToPrimitive, CheckedDiv}; 
...
// --- JSON Instruction Structs (V1 + Addition) ---
...
enum Instruction {
    ...
    // New Instruction Type for Addition
    #[serde(rename = "EVALUATE_ADDITION")]
    EvaluateAddition {
        operand1_value: String, // Operand as a BigInt string
        operand2_value: String, // Operand as a BigInt string
    },
}
...
impl AppState {
    ...
    // --- MODIFIED executeJsonInstructionsToCI to include EVALUATE_ADDITION ---
    ...
    pub fn execute_json_instructions_to_ci(&self, json_string: &str) -> Result<JsBigInt, JsValue> {
        console::log_1(&format!("execute_json: Received JSON string: {}", json_string).into());
        let instruction: Instruction = serde_json::from_str(json_string)
            .map_err(|e| JsValue::from_str(&format!("JSON parse error: {}", e)))?;
        console::log_1(&format!("execute_json: Parsed instruction: {:?}", instruction).into());
        ...
        match instruction {
            Instruction::LiteralBigInt { value } => {
                ...
                if bi.sign() == Sign::Minus { // CI must be non-negative
            ...
            Instruction::EvaluateAddition { operand1_value, operand2_value } => {
                console::log_1(&format!("execute_json: Evaluating Addition: {} + {}", operand1_value, operand2_value).into());
                let op1 = BigInt::from_str(&operand1_value)
                    .map_err(|e| JsValue::from_str(&format!("ADDITION op1 parse error: {}", e)))?;
                let op2 = BigInt::from_str(&operand2_value)
                    .map_err(|e| JsValue::from_str(&format!("ADDITION op2 parse error: {}", e)))?;
                
                // For CI, operands should ideally be non-negative if they represent CIs themselves,
                // but raw addition supports signed numbers. Result must be non-negative for CI.
                let sum = op1 + op2;
                if sum.sign() == Sign::Minus {
                    return Err(JsValue::from_str("ADDITION result cannot be negative for CI."));
                }
                JsBigInt::from_str(&sum.to_string())
                    .map_err(|e| JsValue::from_str(&format!("ADDITION result to JSBigInt fail: {:?}", e)))
            }
        }
    }

    #[wasm_bindgen(js_name=generateJsonAnalysisReportForCurrentCI)] pub fn generate_json_analysis_report_for_current_ci(&self,strat_str:String)->Result<String,JsValue>{ /*..same as before..*/console::log_1(&format!("generateReport:CI_M_target={}",self.canonical_index.to_string()).into());let ci_target_s=self.canonical_index.to_string();let mut analyses:Vec<JsonValue>=Vec::new();let mut best_c:usize;let mut rec_instr_json_val:JsonValue;let lit_bi_instr=json!({"instruction_type":"LITERAL_BIGINT","value":ci_target_s.clone()});let lit_bi_instr_s=serde_json::to_string(&lit_bi_instr).map_err(|e|JsValue::from_str(&format!("SerLitBIGINTFail:{}",e)))?;best_c=lit_bi_instr_s.len();rec_instr_json_val=lit_bi_instr.clone();analyses.push(json!({"lens_id":"LITERAL_BIGINT","instruction":lit_bi_instr.clone(),"estimated_cost":best_c}));console::log_1(&format!("generateReport:LensLITERAL_BIGINTcost:{}",best_c).into());match self.index_to_text_simple(){Ok(curr_txt)=>{let txt_to_use=if curr_txt.is_empty(){PADDING_CHAR.to_string()}else{curr_txt};let lit_txt_instr=json!({"instruction_type":"LITERAL_TEXT_TO_CI","text_value":txt_to_use.clone(),"text_modality_alphabet_id":"SIMPLE_TEXT_A_Z_SPACE"});let lit_txt_instr_s=serde_json::to_string(&lit_txt_instr).map_err(|e|JsValue::from_str(&format!("SerLitTEXTFail:{}",e)))?;let curr_c_txt_lit=lit_txt_instr_s.len();analyses.push(json!({"lens_id":"LITERAL_TEXT_A_Z_SPACE","instruction":lit_txt_instr.clone(),"estimated_cost":curr_c_txt_lit}));console::log_1(&format!("generateReport:LensLITERAL_TEXTcost:{}",curr_c_txt_lit).into());if curr_c_txt_lit<best_c{best_c=curr_c_txt_lit;rec_instr_json_val=lit_txt_instr.clone();console::log_1(&"generateReport:LITERAL_TEXTnewBest.".into());}if let Some((ptrn,cnt))=find_simple_repetition(&txt_to_use){if cnt>1{let rep_txt_instr=json!({"instruction_type":"REPEAT_TEXT_PATTERN_TO_CI","pattern_text":ptrn.clone(),"count":cnt,"text_modality_alphabet_id":"SIMPLE_TEXT_A_Z_SPACE"});let rep_txt_instr_s=serde_json::to_string(&rep_txt_instr).map_err(|e|JsValue::from_str(&format!("SerREPEAT_TEXTFail:{}",e)))?;let curr_c_txt_rep=rep_txt_instr_s.len();analyses.push(json!({"lens_id":"REPEAT_TEXT_A_Z_SPACE_P_N","instruction":rep_txt_instr.clone(),"estimated_cost":curr_c_txt_rep}));console::log_1(&format!("generateReport:LensREPEAT_TEXTcost:{}.P:'{}',N:{}",curr_c_txt_rep,ptrn,cnt).into());if curr_c_txt_rep<best_c{best_c=curr_c_txt_rep;rec_instr_json_val=rep_txt_instr.clone();console::log_1(&"generateReport:REPEAT_TEXTnewBest.".into());}}}}Err(e)=>{console::log_1(&format!("generateReport:Err CI to txt for analysis,skip txt lenses.Err:{:?}",e).into());}}let report=json!({"ci_analyzed":self.canonical_index.to_string(),"analysis_by_lens":analyses,"recommended_instruction_for_save":rec_instr_json_val});console::log_1(&format!("generateReport:FinalRecInstrType:{}",report["recommended_instruction_for_save"]["instruction_type"]).into());serde_json::to_string_pretty(&report).map_err(|e|JsValue::from_str(&format!("FailSerFinalReport:{}",e)))}
}
...
// calculate_min_sequence_length_internal - these are the same as the last full src/lib.rs,
// ensure they are present and correct in your file. Using minified versions here for brevity.)
