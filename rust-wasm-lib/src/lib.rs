...
use js_sys::{BigInt as JsBigInt}; // Removed JsString as it's not directly used for return types now
use web_sys::console;
use num_bigint::{BigInt, Sign};
use num_traits::{Zero, One, Signed, ToPrimitive, Pow};
use anyhow::{Result, bail, anyhow};
use lazy_static::lazy_static;
use serde::{Serialize, Deserialize}; // Used by serde_wasm_bindgen
use std::str::FromStr;
...
// Max bit depth supported for sequence elements.
// Can be increased if performance is acceptable and JS side handles very large BigInt strings.
const MAX_SEQUENCE_ELEMENT_BIT_DEPTH: u32 = 256; // Matches JS CONFIG
...
impl AppState {
    ...
    pub fn get_canonical_index(&self) -> JsBigInt { // Returns JS BigInt
        // num_bigint::BigInt to String, then String to js_sys::BigInt via JsValue
        JsBigInt::from(JsValue::from_str(&self.canonical_index.to_string()))
    }

    #[wasm_bindgen(js_name = setCanonicalIndex)]
    pub fn set_canonical_index(&mut self, js_index: JsBigInt) -> Result<(), JsValue> { // Takes JS BigInt
        let index_str = String::from(js_index.to_string(10).map_err(|e| JsValue::from_str("Failed to stringify JS BigInt"))?);
    ...
    // Now expects an array of JS BigInts, maps to Vec<num_bigint::BigInt>
    ...
    pub fn set_sequence_data(&mut self, js_sequence_array_of_bigints: JsValue, bit_depth: u32) -> Result<(), JsValue> {
        ...
        // serde_wasm_bindgen maps Array<js_sys::BigInt> to Vec<num_bigint::BigInt>
    ...
    // Returns Vec<num_bigint::BigInt>, which maps to Array<js_sys::BigInt>
    ...
    pub fn get_sequence_representation(&self, target_length: u32, bit_depth: u32) -> Result<JsValue, JsValue> {
        ...
        match index_to_sequence(&self.canonical_index, target_length, bit_depth) {
            Ok(sequence_of_bigints) => Ok(serde_wasm_bindgen::to_value(&sequence_of_bigints)?),
            Err(e) => Err(JsValue::from_str(&format!("Sequence Error: {}", e))),
        }
    }
...
fn sequence_to_index(sequence_array: &[BigInt], bit_depth: u32) -> Result<BigInt> {
    ...
    let mut index = BigInt::zero();
    for val_bigint in sequence_array { // val_bigint is already a num_bigint::BigInt
...
fn index_to_sequence(index: &BigInt, target_length: u32, bit_depth: u32) -> Result<Vec<BigInt>> {
    ...
    for _ in 0..target_length {
        ...
        decoded_sequence.insert(0, remainder_bigint); // Insert the BigInt directly
