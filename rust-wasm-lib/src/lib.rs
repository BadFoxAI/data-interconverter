...
use js_sys::{BigInt as JsBigInt, Array as JsArray}; // Ensure JsArray is imported
use web_sys::console; // Keep for console_error_panic_hook if enabled
...
fn index_to_text(index: &BigInt) -> Result<String> {
    ...
    while temp_index > BigInt::zero() {
        ...
        if remainder < CHAR_SET_ARRAY.len() {
            ...
            text.insert(0, '?');
            // web_sys::console::warn_1(&format!("Index to text: Char index {} out of bounds (len {}).", remainder, CHAR_SET_ARRAY.len()).into());
        }
